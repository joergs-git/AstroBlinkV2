...
Some frames fail plate-solving (clouded, defocused, low star count) or pre-date when plate-solving was enabled in the capture pipeline. AutoRotate falls through a layered pipeline (v5.28.0):

1. **PIERSIDE** — if the pier differs from the reference, a clean 180° rotation is applied. Reliable within and across sessions because pier side is a physical mount state.
2. **Star-centroid mirror** — unweighted mean of detected star positions; if the frame's centroid maps closer to the 180°-mirrored reference centroid than to the reference itself, the frame is flipped. Triggers only when the reference is clearly off-center (≥ 8% from image center) so symmetric star fields don't produce false positives.
3. **Pixel fingerprint mirror** — 32×32 spatial signature built during decode (< 1 ms per frame). SAD direct vs. SAD of the 180°-rotated fingerprint; flip when the mirrored match is ≥ 20% better AND the direct SAD is above a low-structure floor. Works on RASA setups (no PIERSIDE, no rotator) and handles rotation-invariant star fields where triangle-based star matching otherwise returns spurious identity matches.

The rotator-delta fallback that existed in v5.27 and earlier was removed in v5.28 — an empirical survey of the local 6 210-frame DB showed rotator encoders take 7–13 distinct positions per target across sessions (from manual recalibrations, home-resets, camera re-mounts), making cross-session rotator deltas unreliable as a rotation signal. The pipeline above is purely physical (mount state, pixel content) and handles the same cases without the cross-session failure mode.

**Q: Why do frames from different setups (e.g. native focal length vs. a focal reducer) not align across each other?**
Each physical rig has its own rotator-encoder zero and its own plate-solve quality. Mathematically, the only fully reliable cross-setup orientation signal is a WCS plate-solve on BOTH sides — without that, the rotator values from two setups can't be compared directly. AstroBlink v5.28 groups orientation per `target | telescope | focal_length | camera` so each rig is internally consistent; cross-setup comparison will render each group in setup-local orientation. If you blink a single-setup session (the common case), everything aligns; if you load mixed setups, expect the two groups to look differently oriented until you cross-reference them manually.
