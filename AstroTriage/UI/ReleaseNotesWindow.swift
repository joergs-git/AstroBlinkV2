...
private let allReleases: [(version: String, date: String, items: [(ReleaseNotesView.ChangeType, String, String)])] = [
    ("6.9.1", "September 15, 2026", [
        (.fixed, "Plate-Solve Result Shows Skipped Frames", "Running plate solving over a session where most frames already carry a solve, and choosing \"Only Unsolved\", reported something like \"Solved 6 of 6 frames (100%)\" with no mention of the several hundred frames skipped because they were already solved. The result read as if the session had shrunk. The skipped count now appears in the summary line, as a chip in the result window, and in the clipboard export."),
    ]),
    ("6.9.0", "September 15, 2026", [
        (.added, "Plate Solving With ASTAP", "AstroBlink can now plate-solve a session's frames — Window → Plate Solve Frames… (Cmd+Shift+P), or right-click a selection in the file list. About 0.3 seconds per frame, because AstroBlink hands ASTAP the field size it already knows from your focal length and pixel size. Frames that already have a solve can be re-solved as a check: the new solution is compared against the stored one and any that disagree are flagged, which is how a stale or misidentified solve becomes visible. The result table names the object each frame points at, shows the solved coordinates, and warns when FOCALLEN disagrees with the measured scale — a forgotten reducer shows up immediately. Clicking a row shows that frame. Nothing is written until you decide after the run whether to save all, only the changed frames, or none; saving always creates a backup first and never overwrites an OBJECT name you already have. Requires ASTAP and a star database to be installed — both free, and licensing prevents shipping them inside AstroBlink. Works with both FITS and XISF."),
        (.fixed, "Backup Folders Were Loaded Back Into The Session", "Any folder AstroBlink creates before modifying your files — the new _platesolve_backup_, and since 6.5.0 also _batch_backup_ and _filter_backup_ — was scanned like a normal subfolder the next time you opened that session. Every modified frame then appeared twice: once current, once as it was before the change. The session doubled, the extra copies were scored alongside the real ones, and because they still carried their old headers they looked like frames that permanently disagreed with themselves. Backup folders are now skipped. It is worth looking through your session folders for old _batch_backup_ and _filter_backup_ directories."),
        (.added, "Progress And Cancel While Plate Solving", "A long run used to show nothing but a line in the status bar. A panel now shows how many frames are done, which frame is being worked on right now — so a stall is visible as a stuck name rather than a stuck bar — and a Cancel button that really stops the run. Frames already in flight finish, nothing further starts, and the result says plainly that the run was cancelled."),
        (.added, "Announcements Can Be A Popup With A Video", "Until now every in-app announcement rendered as the slim banner at the top of the window. A message can now be shown as a proper popup instead, and can carry a short YouTube or Vimeo clip — useful for showing a new feature rather than describing it. An \"Open in browser\" button below the player takes the video full size in your normal browser, where it also outlives the popup. Videos load from the privacy-preserving YouTube domain, nothing the player stores survives closing the popup, and a link clicked inside the player opens your normal browser rather than navigating inside AstroBlink."),
        (.changed, "Announcements Arrive The Same Day", "Messages were only re-fetched every 24 hours, so a new announcement could take a full day to reach you. AstroBlink now checks once at every launch and keeps the 24-hour interval only for the periodic check while the app stays open."),
    ]),
    ("6.8.1", "September 10, 2026", [
        (.fixed, "AIsaac Can Now Explain A Frame's Tier", "\"Ask AIsaac about this frame\" asked AIsaac to explain the z-scores, but it only ever received the combined z-score and had to guess which metric drove the tier. It now gets the same breakdown the Header Inspector shows — per-metric z-scores for stars, FWHM, HFR, noise, trailing and PSF flux — plus the trailing direction consensus and star-chain fraction that every elongation rule keys on, the calibration-lock and low-confidence flags, the session-sanity and historical-baseline reasons, and the recommendation label."),
        (.fixed, "AIsaac's Algorithm Knowledge Updated To v41", "Its built-in description of the scoring rules was three months old: dark frames keyed on 10000 hot-pixel \"stars\", the absolute trailing ceiling still gated on the group, tracking hops on a 25% chain fraction, and the pre-6.7.2 auto-mark levels. Rewritten to the current algorithm — hot-pixel gate, flat-pedestal dark rule, elongation as an absolute defect, direction-based hop detection, plausibility corridor, background floor, frame-of-reference auto-mark — with a version history back to v29 so it can also explain older Frame History records. The same update reaches 6.7.0-6.8.0 installs via remote knowledge."),
        (.fixed, "AIsaac Header List No Longer Cut Alphabetically", "AIsaac received the first 40 header keys in alphabetical order, so on NINA and ASIAIR files OBJECT, PIXSIZE, RA/DEC, ROTATOR, SITELAT/SITELONG, TELESCOP and XBINNING were regularly missing. Headers are now ordered by relevance — target, filter, optics, sensor, pointing, site, environment first — and the cap is 60."),
    ]),
    ("6.8.0", "September 9, 2026", [
        (.fixed, "Hot Pixels No Longer Counted As Stars (Algorithm v41)", "Requested from live testing on an ASI6200: hot-pixel frames reported 10000-25000 \"stars\" where the same frames carry about 2500 real ones, and that one flaw was the root of three problems fixed separately in 6.7.2 and 6.7.3. Neither star detector required a detection to have any spatial extent — a hot pixel is trivially a local maximum and maximally sharp. Every candidate is now checked on the original, unbinned pixels: light from a star spreads in two dimensions, so the pixel above or below the peak AND the pixel left or right of it must each carry a real share of its brightness. Single hot pixels, and the adjacent pairs and short chains this sensor produces, extend along one axis only and are rejected. On the 464-frame calibration set: frames reporting over 10000 stars 30 → 0, frames whose star shapes could not be measured 70 → 3, bad frames caught 147 → 158, good frames wrongly flagged unchanged at 4."),
        (.fixed, "Dome-Closed Frames Scored Good Once The Phantoms Were Gone", "The dark-frame rules keyed on a star count of 10000 or more — and on the RASA those counts had been hot pixels. A frame whose background carries no spatial structure at all (a flat pedestal, the physical signature calibrated in 6.7.1) is now a dark frame on its own, no star count required. All 62 dome-closed frames of the affected night are caught again."),
        (.fixed, "Hazy Frames: Caught By Measurement Instead Of By Accident", "18 hazy OIII frames of one night had been flagged \"no signal detected\" only because the shape measurement failed on them. They now measure 40-370 real stars against 800-3300 on the same night and are flagged \"zero/near-zero stars\". On an unculled second night the fix surfaces six frames whose stars are all trailed diagonally in one direction — crop mosaics confirm the defect. One-shot-colour cameras: detection runs on the debayered green channel, where a hot pixel becomes a small cross; the new check catches about half of those, a complete fix needs a test on the raw colour data."),
    ]),
    ("6.7.3", "September 7, 2026", [
        (.fixed, "Frames With Near-Linear Stars Still Passing (Algorithm v40)", "A follow-up to the fix below. Rescuing just enough star shapes to compute an average was not enough to analyse their DIRECTION — the trailing analysis needs a real sample before it reports anything — so a frame whose stars measured as almost straight lines (eccentricity 0.98) still reported no trailing at all and passed every check, including Aggressive. Seven frames of one reported night sat in that gap. The rescue now collects a sample large enough for the analysis that consumes it."),
        (.fixed, "Egg-Shaped Stars Not Detected On Frames With Sensor Artefacts (Algorithm v39)", "Shape measurement uses the brightest detections in a frame, and on a sensor with hot pixels those are the hot pixels — single bright dots that get discarded as artefacts, leaving too few real stars to measure. Eccentricity then came out as \"not measured\", and every elongation check silently had nothing to look at, so a clearly trailed frame appeared flawless. On the reported session 12 of 21 OIII frames had no shape measurement at all; once real stars are reached they measure eccentricity 0.86-0.90. A rescue pass now looks further down the list, but only for frames where the normal measurement failed — frames that already worked keep exactly the same result."),
        (.fixed, "Good Frames At Long Focal Length Flagged As Tracking Hops (Algorithm v39)", "With shapes finally being measured, frames whose stars are elongated by ordinary optics and seeing were reported as mount errors. What distinguishes a real tracking hop is that every star moves in the SAME direction; optical elongation points every which way. That direction agreement is now required, which separates the two cleanly — confirmed hops sit at consensus 0.50 and above, the affected good frames at 0.22-0.49."),
        (.fixed, "Frames With No Stars At All Were Scored As Good", "Reported from live testing: frames containing nothing — not one star — were rated \"Good\" and survived even the Aggressive level. A star count only counts as measured when the shape measurement actually succeeded; the raw detector output on an empty frame is hot-pixel noise. Four places in the app assigned that raw count unconditionally, so an empty frame carried a plausible star count, and the \"no signal detected\" check has an exception for frames that clearly do have signal — which the fabricated count satisfied. The headless path used for calibration always did this correctly, which is why the calibration set never showed the problem while the app did. All five places now agree."),
    ]),
    ("6.7.2", "September 7, 2026", [
        (.fixed, "Elongated Frames Not Detected When A Whole Night Was Affected (Algorithm v38)", "Star elongation could only be flagged if a frame was trailed more than its own group — so a mount problem lasting a whole night went undetected, because the night's own median was trailed and nothing stood out. One reported frame was clearly trailed (direction consensus 0.91, eccentricity 0.72) yet measured BELOW its neighbours and was passed over. The absolute rule now judges the frame itself: strong elongation with a consistent direction is mount motion, regardless of what the rest of the night looks like. Optical aberration still passes, because it produces random star angles rather than one shared direction — which is what protects fast optics. On the reported session this detects 17 additional Ha and 22 additional SII frames."),
        (.fixed, "Narrowband Frames With Thousands Of Stars Flagged As Having None (Algorithm v38)", "The star-count floor compares a frame against the best frames of its group, and frames carrying hot-pixel sensor artefacts — which report 10-20k \"stars\" — pushed that bar above genuine frames. A reported frame with 2257 real stars was called \"zero/near-zero stars\". On narrowband the target is nebulosity and stars are incidental; the neighbouring rule already accounted for that, this one did not. Now consistent."),
        (.fixed, "Conservative Auto-Mark Only Removes Real Defects", "Reported from a live 833-frame session: Conservative was marking frames with no defect at all — in one case 11 of 19 frames of a night, 9 of them scoring ABOVE their own group average, purely because a different night that happened to be loaded had better seeing. The three levels used to differ only in how far down the quality list they reached, so \"this frame has no stars\" and \"this frame is not among the session's best\" were indistinguishable. They now differ by what each level may compare against: Conservative takes only frames with an actual defect (no stars, trailing, tracking hops, clouds, gradient, low SNR — judged absolutely or against the frames you loaded), Balanced adds the weakest frames of what you loaded, and Aggressive additionally judges against your earlier nights. On the curated calibration set the selection is unchanged; on the reported session it releases 20 of 76 Ha and 14 of 82 SII frames."),
        (.fixed, "Good Frames Flagged As Abnormal Background On Stable Nights (Algorithm v37)", "The background check measures how far a frame deviates from its group. When a night is so stable that most frames report an identical background level, that comparison had nothing left to divide by, and a difference of 3 ADU out of 65535 registered as a large anomaly. The comparison now has a lower bound that scales with the group's own sky brightness, so it follows filter and exposure instead of being a fixed number. Genuine outliers are unaffected: a frame at 9x the background of its own night is still caught."),
    ]),
    ("6.7.1", "September 6, 2026", [
        (.fixed, "Good Narrowband Frames Wrongly Marked As Dome-Closed (Algorithm v36)", "Reported from a live 833-frame session: on RC12 narrowband data, 92 of 244 perfectly good SII frames were flagged \"noise peaks, not real stars (dome/cap)\" — including frames with visible star trailing, which obviously are not dome-closed. The dark-frame detector judges whether a frame's background carries any structure, and the limit it used was a fixed number calibrated on three telescopes. Narrowband at long focal length produces a much smoother background, so the whole distribution sits lower and that fixed number landed in the middle of the normal range instead of below it. It now sits between real dark frames and the quietest genuine frame measured on any setup. No other frame type is affected: on the 464-frame calibration set every result is identical to 6.7.0, all dark frames are still caught, and the one genuine dark frame in the reported session is still detected."),
    ]),
    ("6.7.0", "September 5, 2026", [
        (.fixed, "Dome-Closed and Cloud Frames No Longer Scored Good (Algorithm v33)", "An entire night of dome-closed frames could be rated good: 56 of 62 carried no garbage reason at all. Three separate causes, all the same mistake — a check that trusted something other than the frame's own pixels. (1) The \"has signal\" guard used SNR, which is background level divided by background scatter, and is therefore HIGHEST on a flat frame with no sky at all — dark frames measured 113 against 20 for good frames, so the dark detector never fired. (2) A failed star fit returns a width of exactly 0, which was treated as a real measurement, letting featureless frames slip past every star-based rule at once. (3) Garbage detection read the FWHM that the capture software writes into the filename; a dome-closed frame still carries a plausible-looking value. Measured on a curated 464-frame calibration set across three telescopes, dark-frame catch went from 36% to 100% and cloud from 36% to 90%."),
        (.fixed, "Good Frames Wrongly Flagged As Severely Defocused (Algorithm v34)", "In a group containing many unusable frames, ALL the good frames could be marked \"severe defocus\" — the more garbage a group contained, the more certainly its good frames were flagged. Failed star measurements report a width of 0, and those zeros were counted when computing the group's typical star size, dragging it down until every intact frame looked twice as wide as normal. In one real case, 18 cloud frames pulled the group median from 3.7 down to 1.4 and all 11 good frames were flagged. Failed measurements are now simply absent instead of counted as zero. False alarms in the Conservative auto-mark mode dropped from 32 to 4 out of 206 good frames."),
        (.added, "Physical Plausibility Limits For Star Measurements (Algorithm v35)", "A star cannot be sharper than the atmosphere and your sampling allow. The app now derives that limit for your specific setup from focal length and pixel size — no lookup table, so it scales smoothly from a small smart telescope to a long-focal-length SCT — and discards measurements below it as failures rather than treating them as exceptionally sharp stars. Both limits are configurable rather than hard-coded. Calibrated so that no good frame across three very different telescopes was affected."),
        (.changed, "More Accurate Quality Comparisons Between Frames", "The headless scoring tool used for calibration was measuring with a different image-analysis chain than the app itself, which meant tuning was done against numbers the app never produced. Both now run the identical pipeline."),
    ]),
    ("6.6.0", "July 29, 2026", [
        (.fixed, "Fewer Good Frames Wrongly Flagged (Algorithm v32)", "Clean, good frames in a small per-night group were being demoted to the blue \"uncertain\" tier purely because the group contained few frames — the two-pass night grouping can split one filter's frames across nights, and a small night group tripped the uncertain rule even for frames squarely in the good z-band. Measured on a curated multi-scope calibration set, this cut the good-frame false-alarm rate on clean nights from ~20% to ~5% (on one clean RC12 group, 6 wrongly-flagged → 0). Uncertainty is now about low quality confidence, not small sample size: it applies only to genuinely borderline frames, never to good ones. Bad-frame catch rate is unchanged."),
        (.added, "Golden-Set Curation Tool", "Build a good/bad quality calibration set from inside the app instead of sorting folders by hand. Right-click frames → Golden Set ▸ to tag them Good or a defect (Trailing / Cloud / Gradient / Defocus / Low SNR / Dark-Dome / Bad Star); an optional GS column shows the labels. Then Export Golden Set… (Advanced menu) copies the labeled frames — grouped by scoring group, good frames shared across the group — into a clean <case>/good + <case>/PRE-DELETE folder tree with a metrics manifest. Originals are only ever copied, never moved or deleted. Golden Set Coverage… shows how much material you have per group."),
    ]),
    ("5.28.0", "April 22, 2026", [
        (.fixed, "Rotator-Fallback Removed From Auto-Rotate", "The rotator-fallback branch in WCS alignment used to build a display transform from rotator_frame − rotator_ref for frames without plate-solve data — about 80% of non-WCS frames. Cross-session rotator deltas turned out to be unreliable (7–13 distinct rotator positions per target across sessions from manual recalibrations / home-resets), and this transform silently overrode every other signal. Non-WCS frames now fall cleanly through to PIERSIDE / fingerprint logic."),
        (.added, "32×32 Pixel Fingerprint For Orientation Matching", "New OrientationFingerprint: a 32-cell spatial signature computed from the raw pixel buffer during decode (<1ms per frame). Compares bright-spot positions in the frame against the reference in both direct and 180°-rotated orientation. Immune to filter/gain/exposure changes and to rotation-invariant star field failure modes in triangle matching. Used as the last-resort flip detector when headers are silent, and as an independent veto against bad WCS plate-solve transforms."),
        (.added, "Unified Reference Frame Between Header and WCS Paths", "applyWCSAlignment now overwrites targetOrientationRefs with the frame it picked as WCS reference (smart median CRVAL). Previously the header path and the WCS path used different reference frames of the same target, causing WCS-aligned frames and PIERSIDE-flipped frames to render in different orientations — visible as a 180° offset between the two groups while blinking."),
        (.added, "Plate-Solve Sanity Checks", "Two new sanity nets: (1) within-night + same-rotator frames can't have ≥45° WCS rotation — reject as bad plate solve; (2) if alignment transform rotation ≥45° but pixel fingerprint says the real relationship is a 180° mirror, the transform is wrong by ~90° and we force a clean 180° flip. Catches both rotator-encoder false-positive plate solves and cross-session WCS errors."),
        (.changed, "Per-Setup Orientation Grouping", "Each physical rig (telescope + focal length + camera) now gets its own orientation group, so within one setup everything is internally consistent. Cross-setup comparison of the same sky target will render each setup in its own orientation — a limitation we hit when there's no plate-solve data on one side. Within a single capture setup (the most common case) everything is aligned."),
    ]),
    ("5.27.0", "April 22, 2026", [
        (.fixed, "Auto-Rotate Misfire on Rotation-Invariant Fields", "When frame headers clearly indicated a meridian flip (PIERSIDE differed from the per-target reference, or ROTATOR / WCS rotation ≥ 90°), but the star-matching aligner landed on a spurious near-identity match — which happens on rotation-invariant fields like galaxies and uniform star backgrounds because triangle ratios are rotation invariant — the transform overrode the header decision and the frame rendered un-flipped. Fix: the transform's rotation is now cross-checked against the header signals; if they disagree, we discard the transform and do a clean 180° flip based on the headers. Concrete case: NINA sessions with RC12 + ASI6200MM + Pegasus rotator where PIERSIDE East / West frames rendered inconsistently while blinking."),
        (.added, "Centroid-Mirror Flip Detection", "Auto-rotate gains a 4th fallback signal for meridian flips on frames where PIERSIDE, ROTATOR and WCS are all silent or missing and star matching fails. A 180° rotation maps the reference's star centroid to the mirrored position; when the frame's centroid lands significantly closer to that mirror than to the original (and the reference is clearly off-center, so symmetric star fields don't trigger), a flip is declared. Uses the same star list already computed during prefetch — no new compute cost."),
    ]),
