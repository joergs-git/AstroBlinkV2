    ...
    func loadSession(url: URL) {
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            ...
            entries.withUnsafeMutableBufferPointer { buffer in
                // concurrentPerform writes buffer[idx] from different workers,
                // but each worker writes only its own index (no overlap), so
                // the share is data-race-free. The Swift 6 sendable warning
                // about capturing UnsafeMutableBufferPointer is acknowledged
                // and accepted — the previous workaround using
                // `nonisolated(unsafe) let bufferAlias = buffer` was reverted
                // because it was implicated in a NAS-load hang on Xcode 26
                // (concurrentPerform header writes through an aliased pointer
                // misbehaved, leaving headers unwritten and triggering a
                // post-load Task storm). Going back to using `buffer`
                // directly restores the proven pre-wave-3 behaviour.
