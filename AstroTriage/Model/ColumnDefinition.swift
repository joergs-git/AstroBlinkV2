...
struct ColumnDefinition {
    ...
    static let allColumns: [ColumnDefinition] = [
        ...
        // Sky-background column (hidden by default — opt-in via the column picker). Sort to
        // find washed-out (high) or dark/dead (low) frames the SNR column can mask.
    ...
    static func value(for columnId: String, from entry: ImageEntry) -> String {
        switch columnId {
        case "frameNumber":
            // Show shortId from file hash when available, fall back to session index
            if let sid = entry.shortId { return "#\(sid)" }
        ...
        case "skyBg":
            // Sky background as approximate 16-bit ADU. High = washed-out; low = dark/dead.
    ...
    static func headerToolTip(for columnId: String) -> String? {
        ...
        case "frameNumber":
            return "Unique frame ID derived from file content hash.\nFormat: #XX-NNNN — deterministic, same file always gets same ID.\nFalls back to session index if hash not yet computed."
        case "filename":
