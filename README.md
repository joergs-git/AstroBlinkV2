...
- **VLM Check — AI Visual Anomaly Detection** — generates tiled mosaic wallpapers from your session, sends them to Claude Vision for automated detection of ice crystals, dew, clouds, obstructions, and other visual anomalies. Includes deviation maps highlighting frames that differ from the group median. Click any tile to jump to the frame or mark it for deletion. Works out of the box via Supabase edge function, or bring your own Claude API key.
