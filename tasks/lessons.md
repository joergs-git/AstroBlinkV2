...
## [2026-05-11] — AIsaac was guessing because the verdict was withheld from its context
- **Mistake:** When users asked "why is this trash" on Stage 1.5-demoted frames, AIsaac would speculate ("vermutlich Stage 1.5", "möglicherweise..."), occasionally contradict itself, and sometimes recommend the user press "A" (Disagree) on a frame whose `trailingZ` was at the +3.0 σ cap. The user assumed a prompt-engineering problem. It wasn't: `AIsaacContextBuilder.buildFrameMetricsBlock` emitted a `reason` column populated only from `f.garbageReason ?? ""` (Stage 1 hard rules). The `f.reasoning` field (carrying the literal `QualityBreakdown.reasoningText` — "Session sanity: star count far below session norm, trailing far above session norm" etc.) was collected on `FrameMetric` at the call site but never surfaced into the table the LLM actually reads. So AIsaac was working from `tier=trash, z=-0.35, reason=""` and had nothing concrete to cite.
- **Root cause:** `garbageReason` and `reasoningText` are mutually exclusive on a single frame by design (Stage 1.5 only runs when `garbageReasons.isEmpty`), but the table only surfaced the first. Easy to write, easy to forget — the type carried both fields but the serialiser only used one.
- **Rule:** When a UI/LLM context surface emits a "reason" column, audit BOTH the model that builds it and the renderer that emits it. If multiple stage-specific reason fields exist (`garbageReasons`, `reasoningText`, `sessionSanityReasons`, `historicalBaselineReasons`), the renderer must combine them with a clear precedence — or the LLM will face the same blind spot human reviewers do: tier without a "why". And: never let the LLM speculate when the truth was already computed. Strip pipe chars before emitting into a delimited table.
- **Applies to:** Any future AIsaac column additions, any future verdict-bearing surface (status-bar tooltips, NSTableView columns, exports). Cross-check that every QualityBreakdown reason field has a presentation surface.

## [2026-05-11] — A small-pool P90 can be hijacked by a single outlier
- **Mistake:** Stage 1.5 session-sanity computed `starsP90 = stars[count*9/10]` and used `0.4 × P90` as the "far below norm" threshold. In a small multi-night pool (10-20 frames), a single anomalous detection burst (4928 stars on a frame whose peers had ~900–1200) became the P90 and dragged the threshold up to ~2000 stars — flagging every normal frame for "star count far below session norm". Combined with a real trailing flag on the same frame, the 2-flag rule fired and demoted typical frames to trash. The frame's within-group `starsZ` was +0.84 σ ("normal"), making the verdict visually contradictory.
- **Root cause:** P90 is theoretically robust to bad-frame contamination (best decile = what good looks like), but only when the pool is large enough that the 90th-percentile *is* the actual top-decile boundary. In a 10-15 frame pool with one extreme outlier, P90 collapses to "that outlier". The comment defending P90 as "resists contamination from uniformly-bad nights" was right about the bad-night case but blind to the single-outlier case in the opposite direction.
- **Rule:** Any "best-decile" or "best-N%" benchmark on a small pool needs a sanity clamp against pool median — typical: `min(P90_raw, k × median)` for higher-is-better, `max(P10_raw, k × median)` for lower-is-better — with `k` chosen generously (2.5× for stars/SNR is comfortable) so legitimate session-spanning variation passes through but a 4× outlier cannot define the bar. Validate with both the existing 2-flag-cross-night regression AND a new "single-outlier doesn't poison the bench" regression.
- **Applies to:** Stage 1.5 session-sanity benchmarks (now clamped on stars/SNR-P90 in v30). Same caution applies to any future cross-frame benchmarks: median-relative absolute floors first, percentile-based thresholds only after ruling out small-pool contamination. Keep an eye on FWHM/Ecc/Trailing P10 too — currently unclamped because the downward-outlier failure mode is rare, but worth revisiting if a real case appears.

## [2026-05-11] — Snapshot-then-async-reassign is a data race waiting to happen
- **Mistake:** `TriageViewModel.applySortDescriptors` did `var sortedImages = images; sortedImages.sort {…}; DispatchQueue.main.async { self.images = sortedImages }`. Looked clean — the comment even justified the snapshot as a "Publishing changes from within view updates" mitigation. For months it silently dropped 30-50 prefetch metric callbacks per OSC session. Every `recomputeQualityScores → applySortByColumnOrder → applySortDescriptors` cycle captured a stale snapshot, and any `host.images[idx].noiseMAD = …` / `…computedStarCount = …` callback that landed between snapshot capture and main-thread execution was wholesale overwritten. The symptom was always "40-ish unrated frames per session" — different frames each run because the race depends on worker timing. Diagnosed only after instrumenting every stage of the prefetch + QE pipeline and finding that callbacks DID fire successfully for all 249 frames but the data wasn't reaching QE.
- **Root cause:** The SwiftUI "no @Published from view update" constraint motivates deferring the mutation, but the snapshot+reassign pattern is a stale-data write. The correct pattern is to defer (`DispatchQueue.main.async`) and then mutate the live state in place — sort `self.images.sort(by: comparator)` rather than reassign `self.images = sortedSnapshot`.
- **Rule:** Any time you see `var copy = self.someArray; copy.<mutate>; DispatchQueue.main.async { self.someArray = copy }`, treat it as a bug — concurrent writes to `self.someArray[idx]` between snapshot capture and dispatch execution will be silently lost. Fix by sorting / mutating in place on the dispatched closure, OR by applying a permutation (URL-keyed reorder) to the live array.
- **Applies to:** Any `@Published var array` mutated concurrently from background workers via index-based writes. Particularly relevant in AstroTriage's prefetch + scoring pipeline (TriageViewModel, SessionOrchestrator). Watch for similar patterns in NAS load, header enrichment, alignment.

## [2026-05-11] — `FileHandle.standardError.write` is the right tool for per-frame diagnostics under a terminal pipe
- **Mistake:** Earlier sessions used `print()` for per-frame diagnostic logging and learned the hard way that it serialises the app when launched under a `grep`-pipe (see `feedback_verbose_print_under_pipe.md`). This session faced the same need — instrument every stage of a 257-frame load — and reached for the safe pattern from the start.
- **Rule:** For verbose per-frame diagnostics, write short single-line stderr messages via `FileHandle.standardError.write(Data("...\n".utf8))`. Launch with `2>/tmp/smc.log`, then `grep` patterns. Stderr is unbuffered for short writes, separate from stdout, and doesn't block when the user pipes the app's output. Gate behind a `static let verbose: Bool = UserDefaults.standard.bool(forKey: "DebugFooName")` so production users never pay the cost.
- **Applies to:** Any future debugging session where you need to see what's happening per-frame inside the prefetch / measurement / scoring pipeline. The technique generalises beyond this codebase.

## [2026-05-09] — App Store Connect rejects re-uploaded `CFBundleVersion` even after a prior submission was never released
- **Mistake:** During the v6.0.2 release pipeline, the App Store Connect upload of build 91 failed with `"This bundle is invalid. The value for key CFBundleVersion [91] in the Info.plist file must contain a higher version than that of the previously uploaded version [91]."` Memory had recorded v6.0.2 (build 91) as "committed + pushed but NOT yet released" — implying ASC was clean. It wasn't: a prior session must have submitted build 91 to ASC even though no tag, GH release, or notarisation existed. The mismatch forced a hot-bump to build 92 mid-pipeline, splitting the GH artefact (build 91) from the ASC artefact (build 92).
- **Root cause:** ASC tracks `CFBundleVersion` per-app forever. Any prior submission — even one that was rejected, withdrawn, or never published — burns the build number. Memory's per-session "shipped/not shipped" flag isn't authoritative for ASC; only an actual `xcrun altool list-builds` query (or the ASC dashboard) is. The previous handoff treated "no ASC entry that I created this session" as "ASC is at the previous version" and was wrong.
- **Rule:** Before committing the build-number bump for a release, check ASC's recorded build number for the next-version target via `xcrun altool --list-builds --apiKey ... --apiIssuer ...` (or the dashboard). The starting build for `vX.Y.Z` is `max(local CFBundleVersion + 1, ASC last build for any version + 1)`. Never trust memory or git history alone — ASC's view is the only source of truth on `CFBundleVersion` collisions. If the bump must happen mid-pipeline (as it did here), commit the bumped value to the repo so the next release continues from the right number, but keep the GH artefact at the original build (don't re-tag and don't re-publish — it's a no-op user-visible difference).
- **Applies to:** Every macOS / iOS App Store Connect submission for AstroBlink and AstroFileViewer. Particularly relevant when shipping a hotfix soon after a release, where the previous build number was bumped but the prior session may have already submitted a candidate that was withdrawn or rejected.

## [2026-05-04] — `git stash pop` can be a silent no-op for a subset of files when the working tree is non-clean
- **Mistake:** During Step 1 of Patch 2, used `git stash push -u -m step1-wip` to verify pre-existing test failures reproduce on baseline, then `git stash pop`. The pop silently restored only the untracked `SessionOrchestrator.swift` and the pre-existing `.xcscheme` drift; the tracked-file edits to `TriageViewModel.swift` and `project.pbxproj` were NOT re-applied. No conflict marker, no error message. The stash entry was kept, but the files in the working tree did not match the stashed diff.
- **Root cause:** Unclear — possibly an interaction between `git stash push -u` (which captures untracked files) and the working-tree state at pop time. The kept-stash behavior usually indicates a conflict, but `git status` showed no `UU` markers. Reproducible workaround was to use `git stash show -p stash@{0} > /tmp/patch && git apply --include='<paths>' /tmp/patch`.
- **Rule:** For any non-trivial save/restore around an interim git operation, prefer `git stash show -p > /tmp/x.patch` + `git apply` over `stash push/pop`. Or use a feature branch with two commits — far cheaper than debugging a silent half-restore. If `stash pop` ever leaves the stash kept, immediately verify each tracked file's restoration with `git diff HEAD --stat` before continuing.
- **Applies to:** Any verification flow that needs to bisect "is this regression mine or pre-existing?" by stashing local edits, running tests on baseline, then restoring.

## [2026-05-03] — When `supabase` CLI says "account does not have privileges", reach for the Supabase MCP
- **Mistake:** Started the AstroSharper-extension migration work by running `supabase link --project-ref bpngramreznwvtssrcbe`, which failed with "Your account does not have the necessary privileges to access this endpoint." Almost stopped to ask the user to re-login. The user works across multiple Supabase orgs (dialoKI2, klaasjoerg, cloudwatcher, towel42 are all visible to the CLI session; `astroblink` is a separate org bound to a different access token).
- **Root cause:** The local `supabase` CLI uses the access token from the most recent `supabase login`. The user has too many orgs to keep all logins active simultaneously. The CLI's project-list output only shows projects the *current* token can administer — `bpngramreznwvtssrcbe` simply isn't in scope for the current token.
- **Rule:** When the CLI is the wrong tool for a Supabase op on this project, the **Supabase MCP server is bound to a different (broader) credential set** and can administer `bpngramreznwvtssrcbe` directly. Use `mcp__claude_ai_Supabase__apply_migration`, `..._execute_sql`, `..._deploy_edge_function`, `..._list_projects` etc. The MCP path can also create the `community-thumbnails` storage bucket via raw SQL against `storage.buckets` (skip the dashboard/CLI dance). Don't ask the user to re-login when the MCP can do the same job in-session.
- **Applies to:** Any future Supabase migration / edge-function deploy / SQL inspection on `bpngramreznwvtssrcbe`. Also relevant before flagging "blocked on supabase auth" as a stop condition.

## [2026-05-03] — A counter wired into one of three load paths is a silent bug
- **Mistake:** While building the new "Buy me a coffee" dialog (v5.29) we reused the existing `sessionCount` counter and the `checkForReviewPrompt()` call site. The dialog "wouldn't show" despite resetting the prefs and opening a file. The cause was that `checkForReviewPrompt()` had been hooked only into `loadSession(url:)` — the single-folder path — so opening individual FITS files (`loadFiles`) or multiple folders (`loadMultipleFolders`) never bumped the counter. The App Store review prompt had had this same gap silently for many releases; nobody noticed because Apple rate-limits the prompt anyway.
- **Root cause:** Three separate "load a session" entry points each terminating in their own `await MainActor.run` block. The contract "fire per-session side effects here" lived at only one of them. New developers (and future-me) intuit "open session → counter increments" but the code says "specific path → counter increments".
- **Rule:** When adding any per-session side effect (counters, telemetry, history-DB writes, user prompts), grep `loadSession\|loadFiles\|loadMultipleFolders` and add the call to ALL THREE. Better still: extract a `private func didOpenSession(_ entries: [ImageEntry])` helper at the bottom of each `MainActor.run` so future hooks are added in one place. The current fix wires the call into all three paths but stops short of extracting the helper — leaving that as a refactor for the next per-session feature.
- **Applies to:** TriageViewModel, future Frame History DB writes, future analytics, future per-session messaging.
...
## [2026-05-04] — `nonisolated(unsafe) let` on aliased UnsafeMutableBufferPointer broke NAS load on Xcode 26
- **Mistake:** During Patch 3 wave 3 strict-concurrency cleanup, silenced the Swift 6 sendable warning on the `withUnsafeMutableBufferPointer { buffer in concurrentPerform { idx in buffer[idx].field = ... } }` block by adding `nonisolated(unsafe) let bufferAlias = buffer` and routing all writes through `bufferAlias[idx]`. Build succeeded; tests passed; nothing measured runtime correctness.
- **Root cause:** On Xcode 26 / Swift 6.1 toolchain, `concurrentPerform` writes through the aliased pointer apparently did NOT consistently land in the underlying array storage. Headers came back nil for most frames; the post-load WCS-skip optimisation couldn't fire; the per-file `Task { @MainActor [weak self] in ... }` storm into MainActor multiplied by 4-5×; SwiftUI ended up rendering during the storm and produced a flood of "Publishing changes from within view updates" warnings; main thread saturated; beachball.
- **Rule:** When silencing a Swift 6 sendable / concurrency warning via `nonisolated(unsafe)`, `@unchecked Sendable`, or `withCheckedContinuation`, treat it as a runtime-behaviour change. Never trust "build + unit-test green" alone — exercise the affected hot path against the actual runtime (NAS folder load, GPU stack, etc.) before pushing. The cheapest safety net is an exact reproduction case in the test plan.
- **Applies to:** Any future strict-concurrency wave 4 work, especially patterns that wrap an `UnsafeMutableBufferPointer` capture inside `concurrentPerform` or any other `@Sendable` closure boundary. Check that header data is actually written by reading back `entries.contains { $0.someField != nil }` after the synchronous block.
